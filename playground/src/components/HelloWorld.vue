...
const defaultThemes = ['light', 'dark', 'cupcake', 'bumblebee', 'emerald', 'corporate', 'synthwave', 'retro', 'cyberpunk', 'valentine', 'halloween', 'garden', 'forest', 'aqua', 'lofi', 'pastel', 'fantasy', 'wireframe', 'black', 'luxury', 'dracula', 'cmyk', 'autumn', 'business', 'acid', 'lemonade', 'night', 'coffee', 'winter']

const selectTheme = ref('cupcake')
</script>

<template>
  <div flex justify-center w-full space-x-2>
    <button class="btn btn-secondary" @click="toggleDark()">
      <div dark:i-carbon-moon i-carbon-sun />
    </button>
    <select v-model="selectTheme" class="select max-w-xs">
      <option v-for="theme in defaultThemes" :value="theme">
        {{ theme }}
      </option>
    </select>
  </div>
  <div>
    <div w-60vw :data-theme="selectTheme">
      <div class="rounded-box grid gap-3 border p-6">
        <!-- button -->
